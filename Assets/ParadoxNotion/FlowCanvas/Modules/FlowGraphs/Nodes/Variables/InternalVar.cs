...
namespace FlowCanvas.Nodes
{
    ...
    public class RelayValueInput<T> : RelayValueInputBase, IEditorMenuCallbackReceiver
    {
        ...
        protected override void OnNodeInspectorGUI()
        {
            ...
            var relayOutputs = graph.GetAllNodesOfType<RelayValueOutputBase>().ToList();
            if (relayOutputs.Count < 1)
                return;

            GUILayout.Label("Current Var Reference Count: " + relayOutputs.Count);

            for (int i = 0; i < relayOutputs.Count; i++)
            {
                if (relayOutputs[i] != null)
                {
                    if (GUILayout.Button("Node ID: " + relayOutputs[i].ID.ToString()))
                    {
                        NodeCanvas.Editor.GraphEditorUtility.activeElement = relayOutputs[i];
                    }
                }
            }
        }
    ...
    public class RelayValueOutput<T> : RelayValueOutputBase
    {
        ...
        protected override void OnNodeInspectorGUI()
        {
            ...
            if (sourceInput != null)
            {
                ...
                {
                    NodeCanvas.Editor.GraphEditorUtility.activeElement = sourceInput;
                }
            }
        }
