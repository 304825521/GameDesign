        ...
        static void ShowHeader(Node node) {
            ...
            if (!node.showIcon || node.iconAlignment != Alignment2x2.Default)
            {
                if (node.name != null)
                {
                    ...
                    var headerHeight = node.rect.height <= 30 ? 30 : 24;
                    ...
                    var text = string.Format("<b><size=11><color=#{0}>{1}</color></size></b>", "f1ebd7", finalTitle);
                    ...
                    GUILayout.Label(content, StyleSheet.windowTitle, GUILayout.MaxHeight(16));
                }
            }
        ...
        static void HandleEvents(Node node, Event e) {
            ...
            if ( e.type == EventType.MouseDown && GraphEditorUtility.allowClick && e.button != 2 ) {
                ...
                if (e.control)
                {
                    if (node.isSelected)
                    {
                        var t = GraphEditorUtility.activeElements;
                        if (t.Count > 2)
                        {
                            t.Remove(node);
                            GraphEditorUtility.activeElements = t;
                        }
                        else if (t.Count == 2)
                        {
                            t.Remove(node);
                            GraphEditorUtility.activeElements = null;
                            GraphEditorUtility.activeElement = t[0];
                        }
                        else
                        {
                            GraphEditorUtility.activeElement = node;
                    ...
                    else
                    {
                        ...
                        if (t.Count == 0)
                        {
                            t.Add(GraphEditorUtility.activeElement);
                            t.Add(node);
                            GraphEditorUtility.activeElements = t;
                        }
                        else
            ...
            if ( node is IGraphAssignable ) {
                ...
                if ( assignable.subGraphParameter != null ) {
                    ...
                    if ( assignable.subGraph == null ) {
                        ...
                        //SL--
                        if (!Application.isPlaying && GUILayout.Button("CREATE BOUND"))
                        {
                            var newGraph = NestedUtility.CreateBoundNested(assignable.subGraphParameter.varType, assignable, node.graph);

                            if (assignable.subGraphParameter.varType == typeof(FlowCanvas.FlowScript))
                            {
                                if (newGraph == null)
                                    return;
                                newGraph.AddNode<FlowCanvas.Nodes.ConstructionEvent>(new Vector2(-newGraph.translation.x + 300, -newGraph.translation.y + 0));
                                newGraph.AddNode<FlowCanvas.Nodes.EnableEvent>(new Vector2(-newGraph.translation.x + 300, -newGraph.translation.y + 200));
                                newGraph.AddNode<FlowCanvas.Nodes.Finish>(new Vector2(-newGraph.translation.x + 700, -newGraph.translation.y + 200));
                                newGraph.AddNode<FlowCanvas.Nodes.UpdateEvent>(new Vector2(-newGraph.translation.x + 300, -newGraph.translation.y + 400));
                                newGraph.AddNode<FlowCanvas.Nodes.DisableEvent>(new Vector2(-newGraph.translation.x + 300, -newGraph.translation.y + 600));
                            }
                        }
                    }
                    GUILayout.EndVertical();
