...
public static class NestedUtility
{
    ...
    public static void DeleteAllUselessBoundAsset(Graph currentGraph)
    {
        ...
        else //如果是Asset
        {
            if (AssetDatabase.IsSubAsset(currentGraph))
                return;
            allNestedGraphs = currentGraph.GetAllNestedGraphs<Graph>(true).ToList();
            int length = assets.Length;
            for (int i = 0; i < length; i++)
            {
                Object asset = assets[i];
    ...
    public static T CreateBoundNested<T>(IGraphAssignable parent, Graph owner) where T : Graph
    {
        ...
        (newGraph as UnityEngine.Object).name = typeof(T).Name;
        newGraph.Validate();
    ...
    public static Graph CreateBoundNested(Type t,IGraphAssignable parent, Graph owner)
    {
        Graph newGraph;

        newGraph = (Graph)ScriptableObject.CreateInstance(t);
        (newGraph as UnityEngine.Object).name = t.Name;
        newGraph.Validate();
    ...
    public static void SaveToAsset(Graph graph, Object targetMainAsset)
    {
        ...
        if (graph.agent != null)
        {
            ...
            //Debug.Log("all nested graph count:" + allNestGraphs.Count);
            //allNestGraphs.ForEach(x=>Debug.Log("nested graph:"+ x.name));
#if !UNITY_2018_3_OR_NEWER
...
#endif
            {
                ...
                else
                {
                    ...
                    copyPath = targetMainAssetPath;
                    //Debug.Log("copyprefab path:" + copyPath);
                    ...
                    {
                        Debug.LogError("保存错误！----请打开进入当前Prefab（Open Prefab），选择顶层级挂有FlowScriptController组件的物体，再次使用“Save To Asset”保存---");
                        //return;
                    }
#endif
            ...
            else //prefab parent物体模式、noPrefab 或者资源模式 
            {
                foreach (var v in allNestGraphs)
                {
                    if (v != graph && !SaveToAssetCheckHasThisAsset(v, graph))
                    {
                        ...
                        {
                            Debug.Log("如果在prefab模式下保存时，注意检查subgraph："+ v.name + " 引用是否正确");
    ...
    public static void ApplyGraphChange(GraphOwner go, GameObject instance)
    {
        ...
        if (prefabType != UnityEditor.PrefabInstanceStatus.MissingAsset || prefabType != UnityEditor.PrefabInstanceStatus.NotAPrefab)
        {
            ...
            if (!Application.isPlaying)
            {
                ...
                allNestGraphs = go.graph.GetAllNestedGraphs<Graph>(true).ToList();
