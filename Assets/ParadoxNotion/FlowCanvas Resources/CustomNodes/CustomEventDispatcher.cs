...
namespace FlowCanvas.Nodes{
	...
	public class CustomEventListener : RouterEventNode<GraphOwner> {
        ...
        protected virtual void OnCustomEvent(string eventName, ParadoxNotion.IEventData msg)
        {
...
#if UNITY_EDITOR
                if (NodeCanvas.Editor.Prefs.logEventsInfo){
					Logger.Log(string.Format("Event Received from ({0}): '{1}'", receiver.name, eventName), "Event", this);
				}
				#endif
	...
	public class CustomEventListener<T> : CustomEventListener {
		...
		protected override void OnCustomEvent(string eventName, ParadoxNotion.IEventData msg)
        {
            if (eventName.Equals(this.eventName.value, System.StringComparison.OrdinalIgnoreCase))
            {
                var senderGraph = Graph.GetElementGraph(msg.sender);
