//----------------------------------------------
//            Behaviour Machine
// Copyright © 2014 Anderson Campos Cardoso
//----------------------------------------------

using UnityEngine;
using System.Collections;
using UnityEngine.UI;

namespace BehaviourMachine {

    /// <summary>
    /// Changes the texture of a Image.
    /// </summary>
    [NodeInfo(  category = "Action/GUIElement/",
                icon = "Image",
                description = "Changes the texture of a Image",
                url = "http://docs.unity3d.com/Documentation/ScriptReference/SetImage.html")]
    public class SetImage : ActionNode {

        /// <summary>
        /// The game object that has a gui texture in it.
        /// </summary>
        [VariableInfo(requiredField = false, nullLabel = "Use Self", tooltip = "The game object that has a gui texture in it")]
        public GameObjectVar gameObject;

        /// <summary>
        /// The new texture to display.
        /// </summary>
        [VariableInfo(requiredField = false, nullLabel = "Don't Change", tooltip = "The new texture to display")]
        public TextureVar newTexture;

        /// <summary>
        /// The new color of the texture.
        /// </summary>
        [VariableInfo(requiredField = false, nullLabel = "Don't Change", tooltip = "The new color of the texture")]
        public ColorVar newColor;

        [System.NonSerialized]
        Image m_SetImage;

        public override void Reset () {
            newTexture = new ConcreteTextureVar();
            newColor = new ConcreteColorVar();
        }

        public override Status Update () {
            // Get the gui texture
            if (m_SetImage == null || m_SetImage.gameObject != gameObject.Value)
                m_SetImage = gameObject.Value != null ? gameObject.Value.GetComponent<Image>() : null;

            // Validate members
            if (m_SetImage == null)
                return Status.Error;

           // if (!newTexture.isNone) m_SetImage.texture = newTexture.Value;
            if (!newColor.isNone) m_SetImage.color = newColor.Value;
            
            return Status.Success;
        }
    }
}